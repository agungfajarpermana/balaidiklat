...
<html lang="en">
  ...
  <body class="bg-light">
    ...
    <div class="container" style="padding:10px;width:80%;">
      ...
      @if ($page != 0)
        @foreach ($data as $key => $rbpmd)
          <div class="card" style="margin-bottom:5px;">
            <div class="card-header">
              {{$rbpmd->pengajar}}
              <span class="badge {{(Auth::user()) ? (Auth::user()->status == 1 || Auth::user()->status == 2) ? ($rbpmd->status == 1) ? 'badge-danger' : 'badge-primary' : '' : ''}}">
                {{(Auth::user()) ? (Auth::user()->status == 1 || Auth::user()->status == 2) ? ($rbpmd->status == 1) ? 'New' : 'Read' : '' : ''}}
              </span>
              <span class="text-right float-right">{{$rbpmd->nip}}</span>
            </div>
            <div class="card-body">
              <div class="list-group">
                <a href="{{route('rbpmd.detail', $rbpmd->getHashAttribute($rbpmd->id))}}" class="list-group-item list-group-item-action flex-column align-items-start">
                  <div class="d-flex w-100 justify-content-between">
                    <h5 class="mb-1">{{$rbpmd->nama_pelatihan}}</h5>
                    <small>{{$rbpmd->alokasi_waktu}} JP</small>
                  </div>
                  <p class="mb-1">{{$rbpmd->deskripsi_singkat}}</p>
                  <small><span class="text-danger">Tag:</span> {{$rbpmd->mata_pelatihan}}</small>
                </a>
              </div><br>
              <a href="{{route('duplicate.show', $rbpmd->getHashAttribute($rbpmd->id))}}" class="btn btn-outline-danger">Duplicate</a>

              <a href="{{ route('print.rbpmd', $rbpmd->getHashAttribute($rbpmd->id)) }}" class="btn btn-outline-success btn-pdf float-right download{{$rbpmd->getHashAttribute($rbpmd->id)}}" id="download" data-id="{{$rbpmd->getHashAttribute($rbpmd->id)}}">Download PDF</a>
    ...
    <br>
    <br>
    <ul class="nav justify-content-center bg-secondary {{($pages <= 2) ? 'fixed-bottom' : ''}}">
      <li class="nav-item">
    ...
    <script type="text/javascript">
      $(document).ready(function(){
        // $('.btn-pdf').printPage({message:"Mohon Tunggu"});
